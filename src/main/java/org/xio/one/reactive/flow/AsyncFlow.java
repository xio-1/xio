...
import org.xio.one.reactive.flow.events.EmptyEventArray;
import org.xio.one.reactive.flow.events.Event;
import org.xio.one.reactive.flow.events.EventIDSequence;
import org.xio.one.reactive.flow.events.JSONValue;
 ...
 */
public final class AsyncFlow<T, R> {

  // streamContents variables
  private FlowControl<T> eventEventStore;
  ...
  public AsyncFlow(String streamName) {
    ...
    this.eventEventStore = new FlowControl(this);
    this.eventIDSequence = new EventIDSequence();
  }

  public AsyncFlow(String streamName, String indexFieldName) {
    this.event_queue = new LinkedBlockingQueue<>(queue_max_size);
    this.eventqueue_out = new Event[this.queue_max_size];
    this.streamName = streamName;
   ...
   * A future result will be made available immediately the events is processed by the subscriber
   *
   * @return
   */
  ...
  /**
   * Subscribe to every events in the stream with the given single events subscriber
  ...
  /**
   * Subscribe to every events in the stream with the given multiplex events subscriber
   * A final (future) result can be made available when the stream has ended
   *
   * @param subscriber
   * @return
  ...
  public AsyncFlow<T, R> withImmediateFlushing() {
    this.flushImmediately = true;
    return this;
  }

  
  public AsyncFlow<T, R> withExecutorService(ExecutorService executorService) {
