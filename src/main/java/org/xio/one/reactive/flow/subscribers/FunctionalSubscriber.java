...
import org.xio.one.reactive.flow.subscribers.internal.functional.OnExitAndReturnFunction;
import org.xio.one.reactive.flow.subscribers.internal.functional.OnNextFunction;
import org.xio.one.reactive.flow.subscribers.internal.functional.OnNextReturnFunction;
import org.xio.one.reactive.flow.subscribers.internal.functional.OnStartFunction;

public class FunctionalSubscriber<R, T> {

  private final Flow<T, R> trFlow;
  ...
  private class FunctionalItemSubscriber extends ItemSubscriber<R, T> {
    ...
    public void onNext(Item<T> item) {
      if (onExitAndReturn != null && exitPredicate.test(item.getItemValue())) {
