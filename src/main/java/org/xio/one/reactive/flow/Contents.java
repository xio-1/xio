...
import org.xio.one.reactive.flow.events.ComparatorEvent;
...
public final class Contents<T> {
  ...
  public Event<T> event(long id) {
    return eventStoreContents.higher(new ComparatorEvent(id - 1));
  }
      ...
      if (lastEvent != null) {
        ...
        if (events.size() > 0) {
          ...
          if (newFirstEvent.eventId() == (lastEvent.eventId() + 1)) {
            // if last events is in correct sequence then
  ...
  public Optional<Event> nextFollowing(long eventid) {
    return Optional.ofNullable(this.eventStoreContents.higher(new ComparatorEvent(eventid)));
  }
  ...
  /*public Event getMax(String fieldname) {
    try {
      ...
      Event events;
      events = eventRepositoryContents.parallelStream()
          .filter(u -> u.isAlive(eventTTLSeconds) && u.hasFieldValue(fieldname))
