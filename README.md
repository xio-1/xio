...
XIO is small, fast, lightweight and easy to use and has zero dependencies on other frameworks!

To build xio-core go to the directory xio-core ```mvn clean package```

This s/w is still in beta phase and so minor changes are expected to some method signatures, plus known issues when running test from within the IDE rather than Maven.
